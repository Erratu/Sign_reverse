...
def plot_embeddings(
    ...
    for i in range(len(col)):
      ax.scatter(embeds[i,0],embeds[i,1], color = col[i][0])
    
    plt.title(projection+' with '+distance)
    #plt.show()
...
def plot_pca(
    ...
    for i in range(len(col)):
      ax.scatter(np.dot(embeds[0], B[i]), np.dot(embeds[1], B[i]), color = col[i][0])
    
    plt.title("PCA"+' with '+distance)
    #plt.show()


def compute_pers_diag_manifold(B,met,n_comp,col, distance, axs = None):
    ...
    if met == 0:
        for i in range(len(B)):
            ...
            plot_embeddings(mds_embeds,col = col, dim = i, projection = "MDS", distance = distance, ax = axs[i])
    ...
    if met == 1:
        for i in range(len(B)):
            ...
            plot_pca(pca_embeds, B[i], col, distance = distance, ax = axs[i])
    ...
    if met == 2:
        for i in range(len(B)):
            ...
            plot_embeddings(iso_embeds,col = col, dim = i, projection = "Isomap", distance = distance, ax = axs[i])
    ...
    if met == 3:
        for i in range(len(B)):
            ...
            plot_embeddings(tsne_embeds,col = col, dim = i, projection = "tSNE", distance = distance, ax = axs[i])
    ...
    if met == 4:
        for i in range(len(B)):
            ...
            plot_embeddings(lle_embeds,col = col, dim = i, projection = "LLE", distance = distance, ax = axs[i])
    ...
    if met == 5:
        for i in range(len(B)):
            ...
            plot_embeddings(se_embeds,col = col, dim = i, projection = "SpecEmb", distance = distance, ax = axs[i])