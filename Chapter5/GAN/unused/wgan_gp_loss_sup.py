...
class GAN():
    def __init__(self, epochs, batch_size, channels, input_dim, loss_function, lamb, lr_G=1e-4, lr_D=1e-4, betas=(0.0, 0.9), alpha=0.5, gamma=0.1, device="cpu"):
    ...
    def train_step(self, train_data, mean, std, dir):
        ...
        for epoch in range(self.epochs):
            for _, real_samples in enumerate(train_loader):
                ...
                generated_samples = self.generator(latent_space_samples)
                loss_G_wgan = -self.discriminator(generated_samples).mean()

                loss_mean = torch.mean(generated_samples.mean(dim=0)**2)
                loss_std  = torch.mean((generated_samples.std(dim=0) - 1)**2)

                cov_gen = (generated_samples.T @ generated_samples) / (generated_samples.shape[0] - 1)
                cov_real = (real_samples_norm.T @ real_samples_norm) / (real_samples_norm.shape[0] - 1)

                loss_corr = torch.mean((cov_gen - cov_real)**2)
                
                loss_G = loss_G_wgan + self.alpha * (loss_mean+loss_std) + self.gamma * loss_corr
            ...
            if epoch % 10 == 0:
                print(f"Epoch: {epoch} Loss D.: {d_loss} Loss G.: {loss_G}")
        ...
        plt.plot(losses_D, "r.")
        plt.plot(losses_G, "b.")
        plt.savefig(f"./models_saved/results_sup_{dir}.png")
        ...
        plt.show()
        torch.save(self.generator.state_dict(), f"./models_saved/wgan_gp/{dir}/sup_G_model.pt")
        torch.save(self.discriminator.state_dict(), f"./models_saved/wgan_gp/{dir}/sup_D_model.pt")
