...
class SigComplex:
    def __init__(self, MultiTS : torch.tensor, win: int, depth: int = 3, alpha_1d: int = 1, alpha_2d: int = 1, lasso: bool = True, max_iter: int = 2000):
        '''
    ...
    def leadlag(self, X):
        ...
        zeros_beg = np.insert(X,0,0).reshape([1,T+1,1])
        lead_lag_impair = np.concatenate((np.insert(X,T,0).reshape([1,T+1,1]),zeros_beg),axis=-1)
    ...
    def simplex_2d(self,t,verbose = True):
        ...
        k = int(2)
        depth = self.depth
        chansig = signatory.signature_channels(k, depth = self.depth)
        k_uplets = list(combinations(np.arange(self.D),k))
        ...
        df_sig_target = np.zeros([chansig,self.D])
    ...
    def complex_creation(self,t, along_t = False, verbose = True, with_wnorm = False):
        ...
        adj_mat_2d, k_uplets = self.simplex_2d(t,verbose = verbose)
        ...
        for i in range(adj_mat_2d.shape[0]):
            for j in range(adj_mat_2d.shape[1]):
                if adj_mat_2d[i,j] != 0:
                    num2 += 1
                    if not c.find([i,k_uplets[j][0],k_uplets[j][1]]):
                        name = [i,k_uplets[j][0],k_uplets[j][1]]
                        name.sort()
                        id2 =  str(name[0]) + '-' + str(name[1]) + '-' + str(name[2]) 
                        _ = c.insert(name, 
                                     filtration = 1/np.abs(adj_mat_2d[i,j]))  
                        if along_t:
                    ...
                    # when same basis (0-simlex) are found, all coeffients are added to the weight
                        ...
                        _ = c.assign_filtration(name,1/(1/c.filtration(name)+np.abs(adj_mat_2d[i,j])))                 
                    
        if verbose:
            print("Epoch : ",t," with "+str(num2)+" 2d-simplex")
        self.n2D.append(num2)
